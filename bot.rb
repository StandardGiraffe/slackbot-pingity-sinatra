...
# A helper method for sending messages via Slack's Web API
def send_message(team_id:, channel:, ts: nil, text: nil, blocks: nil)
  message = {
    as_user: 'true',
    channel: channel,
    text: text
  ...
  if ts
    message.merge!({ ts: ts })
    $teams[team_id]['client'].chat_update(message)
  else
    $teams[team_id]['client'].chat_postMessage(message)
  end
end
...
class PingityBot
  def self.ping(request_data:, uri:)
    ...
    send_message(team_id: team_id, channel: channel, text:
      "I'm attempting to ping \"#{uri}\".\nJust a moment, please..."
    )
  end
